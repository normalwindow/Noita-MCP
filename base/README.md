...
### 用宏代替手工拼按键时序
- `noita_macro_list` — 24 个命名宏，附每个宏实际发送的按键序列，以及**哪些现在就能跑**。
- `noita_macro` — 跑一个宏（`walk_right`、`jump_right`、`interact`…）。立即返回；桥接每帧推进
  一步，并在结束时**释放所有按键**，所以宏不会把角色留在"一直走"的状态。同时只能跑一个，
  启动新的会干净地取消旧的。
- `noita_macro_status`、`noita_macro_stop`。

**基础版可以直接跑运动宏**：没有输入扩展时，运动宏改用 `lever.lua` 写速度来驱动玩家，
所以基础版能走、能爬、能闪避。24 个宏里 **11 个基础版就能跑**，13 个需要输入扩展
（开火、交互、物品栏——这些必须是真的按键）。

需要按键的宏在没有扩展时会**明确拒绝**，并说明哪一步被挡、为什么、以及现在能用什么替代——
不会假装成功。实测（未加载扩展）：`walk_right` 让玩家移动了 Δx = 62.3。
